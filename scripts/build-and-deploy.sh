...
xcrun devicectl device install app --device 00008130-0004446200698D3A "$APP_PATH" 2>&1 && echo "iPhone: OK" || echo "iPhone: FAILED (not connected?)"

# Deploy to iPad (Fritz)
xcrun devicectl device install app --device 00008020-001079801440402E "$APP_PATH" 2>&1 && echo "iPad: OK" || echo "iPad: FAILED (not connected?)"
